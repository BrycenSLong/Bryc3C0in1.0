UNIX BUILD NOTES
====================
Some notes on how to build Brycecoin in Unix.

(For BSD specific instructions, see `build-*bsd.md` in this directory.)

Note
---------------------
Always use absolute paths to configure and compile Brycecoin and the dependencies,
...
This will build Brycecoin-qt as well, if the dependencies are met.
...
memory available when compiling Brycecoin. On systems with less, gcc can be
...
If you want to build Brycecoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a Brycecoin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip Brycecoind" to strip the debug
...
Security
--------
To help make your Brycecoin installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, Brycecoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, Brycecoin may be compiled in
...
As mentioned above, when maintaining portability of the wallet between the standard Brycecoin distributions and independently built
