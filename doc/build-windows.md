...
Below are some notes on how to build Brycecoin for Windows.

The options known to work for building Brycecoin on Windows are:
...
Note that for WSL the Brycecoin source path MUST be somewhere in the default mount file system, for
example /usr/src/Brycecoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\Brycecoin`, for example:

    make install DESTDIR=/mnt/c/workspace/Brycecoin
