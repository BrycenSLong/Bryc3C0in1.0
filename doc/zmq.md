...
The Brycecoin daemon can be configured to act as a trusted "border
router", implementing the Brycecoin wire protocol and relay, making
...
The ZeroMQ feature in Brycecoin requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building Brycecoind:
...
For instance:
    ...
    $ Brycecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
...
From the perspective of Brycecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into Brycecoind directly. Furthermore, no information is
...
using. Brycecoind appends an up-counting sequence number to each
