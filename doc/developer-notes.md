...
Run with the `-testnet` option to run with "play Brycecoins" on the test network, if you
...
Brycecoin is a multi-threaded application, and deadlocks or other
...
However, in open source software such as Brycecoin, where everyone uses
...
pay attention to for reviewers of Brycecoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Brycecoin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Brycecoin, in which case
...
a Brycecoin subtree, so that they can be integrated quickly. Cosmetic changes
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `Brycecoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Brycecoin, the wallet was always
