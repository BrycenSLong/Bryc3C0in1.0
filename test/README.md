This directory contains integration tests that test bryc3c0ind and its
...
bryc3c0ind and bitcoin-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bryc3c0in-util, bryc3c0in-tx, ...).
...
The P2P and RPC ports used by the bryc3c0ind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bryc3c0ind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bryc3c0ind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bryc3c0ind processes are running.

On linux, the test framework will warn if there is another
bryc3c0ind process running when the tests are started.

If there are zombie bryc3c0ind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bryc3c0ind processes running on the system, so should not be used if any non-test
bryc3c0ind processes are being run.**
...
sure bryc3c0ind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bryc3c0ind `debug.log`s will all be dumped
...
`test_framework.log` and bryc3c0ind `debug.log`s can be combined into a single
...
call methods that interact with the bryc3c0ind nodes-under-test.

If further introspection of the bryc3c0ind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bryc3c0ind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bryc3c0ind <pid>
```
