...
"""Class for bryc3c0ind node under test"""
...
class TestNode():
    """A class for representing a bryc3c0ind node under test.
    ...
    def __init__(self, i, datadir, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        ...
        self.bitcoinconf = os.path.join(self.datadir, "bryc3c0in.conf")
    ...
    def __del__(self):
        # Ensure that we don't leave any bryc3c0ind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # unclean shutdown), it will get overwritten anyway by bryc3c0ind, and
        ...
        self.running = True
        self.log.debug("bryc3c0ind started, waiting for RPC to come up")
    ...
    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the bryc3c0ind process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'bryc3c0ind exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
...
class TestNodeCLI():
    """Interface to bryc3c0in-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.bryc3c0incli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with bryc3c0in-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run bryc3c0in-cli command. Deserializes returned string as python object."""
        pos_args = [arg_to_cli(arg) for arg in args]
        named_args = [str(key) + "=" + arg_to_cli(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same bryc3c0in-cli call"
        ...
        self.log.debug("Running bryc3c0in-cli {}".format(p_args[2:]))
