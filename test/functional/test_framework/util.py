...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "bryc3c0in.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "bryc3c0in.conf")):
        with open(os.path.join(datadir, "bryc3c0in.conf"), 'r', encoding='utf8') as f:
            for line in f:
