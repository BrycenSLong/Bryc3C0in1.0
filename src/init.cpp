 ...
 */
static const char* BITCOIN_PID_FILENAME = "bryc3c0ind.pid";
...
{
    // Make sure only a single Bryc3C0in process is using the data directory.
...
bool AppInitSanityChecks(const kernel::Context& kernel)
{
    ...
    // bryc3c0in: init hash seed
    bryc3c0inRandseed = GetRand(1 << 30);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bryc3c0in is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bryc3c0in is started while in a temporary directory.\n",
