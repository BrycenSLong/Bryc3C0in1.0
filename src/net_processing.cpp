...
private:
    ...
    */
    ...
    /** Brycecoin: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
...

    {
        ...
        if (inv.hash == peer.m_continuation_block) {
            ...
            // Brycecoin: send latest proof-of-work block to allow the
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Brycecoin: tell downloading node about the latest block if it's
...
*/
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        headers.resize(nCount);
        {
            ...
            for (unsigned int n = 0; n < nCount; n++) {
                ...
                // Brycecoin: quick check to see if we should ban peers for PoS spam
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        bool min_pow_checked = false;
        {
            ...
            // Brycecoin: store in memory until we can connect it to some chain
        ...
        // Brycecoin: accept as many blocks as we possibly can from mapBlocksWait
        while (fContinue) {
            fContinue = false;
            ...
            {
                LOCK(cs_main);
                // Brycecoin: try to select next block in a constant time
