...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, Dersig100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        spends[i].vout[0].nValue = 11*scott;
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    spend_tx.vout[0].nValue = 11*scott;
    spend_tx.vout[0].scriptPubKey = p2sh_scriptPubKey;
    spend_tx.vout[1].nValue = 11*scott;
    spend_tx.vout[1].scriptPubKey = p2wpkh_scriptPubKey;
    spend_tx.vout[2].nValue = 11*scott;
    spend_tx.vout[2].scriptPubKey = CScript() << OP_CHECKLOCKTIMEVERIFY << OP_DROP << ToByteVector(coinbaseKey.GetPubKey()) << OP_CHECKSIG;
    spend_tx.vout[3].nValue = 11*scott;
    ...
    // then test validity with P2SH.
    {
        ...
        invalid_under_p2sh_tx.vout[0].nValue = 11*scott;
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        invalid_with_cltv_tx.vout[0].nValue = 11*scott;
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        invalid_with_csv_tx.vout[0].nValue = 11*scott;
    ...
    // for the same tx with a different witness.
    {
        ...
        valid_with_witness_tx.vout[0].nValue = 11*scott;
        valid_with_witness_tx.vout[0].scriptPubKey = p2pk_scriptPubKey;

        // Sign
        SignatureData sigdata;
        BOOST_CHECK(ProduceSignature(keystore, MutableTransactionSignatureCreator(valid_with_witness_tx, 0, 11 * scott, SIGHASH_ALL), spend_tx.vout[1].scriptPubKey, sigdata));
...

    {
        ...
        tx.vout.resize(1);
        tx.vout[0].nValue = 22*scott;
        tx.vout[0].scriptPubKey = p2pk_scriptPubKey;

        // Sign
        for (int i = 0; i < 2; ++i) {
            SignatureData sigdata;
            BOOST_CHECK(ProduceSignature(keystore, MutableTransactionSignatureCreator(tx, i, 11 * scott, SIGHASH_ALL), spend_tx.vout[i].scriptPubKey, sigdata));
