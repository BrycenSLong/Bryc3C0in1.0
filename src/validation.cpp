...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // Brycecoin: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
    // BRYTODO move back to policy.h after 0.8 is active
    //if (IsBTC16BIPsEnabled(tx.nTime))
    //    scriptVerifyFlags &= SCRIPT_VERIFY_LOW_S;

    // Brycecoin allow taproot after fork
    //if (IsProtocolV12(tx.nTime))
    //    scriptVerifyFlags &= SCRIPT_VERIFY_TAPROOT;

    // Brycecoin: verify anyprevout after fork
...
int64_t GetProofOfWorkReward(unsigned int nBits, uint32_t nTime)
{
    ...
    // Brycecoin: subsidy is cut in half every 16x multiply of difficulty
    // A reasonably continuous curve is used to avoid shock to market
    // (nSubsidyLimit / nSubsidy) ** 4 == bnProofOfWorkLimit / bnTarget
    CBigNum bnLowerBound = scott;
    CBigNum bnUpperBound = bnSubsidyLimit;
    while (bnLowerBound + scott <= bnUpperBound)
    {
...
// Brycecoin: miner's coin stake is rewarded based on coin age spent (coin-days)
int64_t GetProofOfStakeReward(int64_t nCoinAge, uint32_t nTime, uint64_t nMoneySupply)
{
    static int64_t nRewardCoinYear = scott;  // creation amount per coin-year
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // Brycecoin
            undo.nTime = alternate.nTime;           // Brycecoin
        } else {
...
bool BrycecoinContextualBlockChecks(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex, bool fJustCheck, Chainstate& chainstate)
{
    uint256 hashProofOfStake = uint256();
    // Brycecoin: verify hash target and signature of coinstake tx
    ...
    // Brycecoin: check for duplicity of stake
    ...
    // Brycecoin: compute stake entropy bit for stake modifier
    unsigned int nEntropyBit = GetStakeEntropyBit(block);

    // Brycecoin: compute stake modifier
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !BrycecoinContextualBlockChecks(block, state, pindex, fJustCheck, m_chainman.ActiveChainstate()))
    ...
    // Brycecoin: coinbase reward check relocated to CheckBlock()
    ...
    // Brycecoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // Brycecoin increment nHeightStake if block is proof of stake
    pindex->nHeightStake = (pindex->pprev ? pindex->pprev->nHeightStake : 0) + block.IsProofOfStake();

    // Brycecoin: fees are not collected by miners as in bitcoin
    // Brycecoin: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // Brycecoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cs-missing", "coinstake in wrong position");

    // Brycecoin: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx) {
        ...
        if (!CheckTransaction(*tx, tx_state)) {
            ...
            // Brycecoin: check transaction timestamp
    ...
    // Brycecoin: check block signature
...
bool Chainstate::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    // Brycecoin: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // Brycecoin: check PoS
    if (fCheckPoS && !BrycecoinContextualBlockChecks(block, state, pindex, false, m_chainman.ActiveChainstate())) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        m_blockman.m_dirty_blockindex.insert(pindex);
...
// Brycecoin: total coin age spent in transaction, in the unit of coin-days.
...
{
    arith_uint256 bnscottSecond = 0;  // coin age in the unit of cent-seconds
    nCoinAge = 0;
    ...
    for (const auto& txin : tx.vin)
    {
        ...
        bnscottSecond += arith_uint256(nValueIn) * nEffectiveAge / CENT;

        if (gArgs.GetBoolArg("-printcoinage", false))
            LogPrintf("coin age nValueIn=%-12lld nTimeDiff=%d bnscottSecond=%s\n", nValueIn, nEffectiveAge, bnscottSecond.ToString());
    }

    arith_uint256 bnCoinDay = bnscottSecond * scott / COIN / (24 * 60 * 60);
    if (gArgs.GetBoolArg("-printcoinage", false))
